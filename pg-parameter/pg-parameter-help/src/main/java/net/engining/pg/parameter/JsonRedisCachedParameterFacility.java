 ...
 * 
 * 注意：<br>
 * 1.由于使用@Cacheable是使用AOP实现的，那么如果存在类内部的方法调用类内部的加缓存注解的方法不会走代理，就不能正常创建缓存，所以每次都需要去调用数据库。<br>
...
public class JsonRedisCachedParameterFacility extends JsonGuavaCachedParameterFacility {

	private static final Logger logger = LoggerFactory.getLogger(JsonRedisCachedParameterFacility.class);
	...
	public <T> T getParameter(Class<T> paramClass, String key) {
		...
		logger.trace("本操作将从数据库加载该参数，如果不是内部方法调用将触发向Redis服务器添加参数缓存； param class:{}, key:{}",
				paramClass.getName(), key);
	 ...
	 * (non-Javadoc)
	 * @see net.engining.pg.parameter.XmlGuavaCachedParameterFacility#getParameter(java.lang.Class, java.lang.String, java.util.Date)
	 */
	@Override
	...
	public <T> T getParameter(Class<T> paramClass, String key, Date effectiveDate) {
		logger.trace("本操作将从数据库加载该参数，如果不是内部方法调用将触发向Redis服务器添加参数缓存；param class:{}, key:{}, effective date:{}",
				paramClass.getName(), key, effectiveDate);
	...
	public <T> T loadUniqueParameter(Class<T> paramClass) {
		logger.trace("本操作将从数据库加载该参数，如果不是内部方法调用将触发向Redis服务器添加参数缓存；param class:{}",
				paramClass.getName());
	...
	public void updateParameter(HasKey parameter) {
		logger.trace("本操作将先到数据库更新该参数，如果不是内部方法调用将触发向Redis服务器删除参数缓存; param key:{}, param class:{}",
				parameter.getKey(), parameter.getClass().getName());
	 ...
	 * 由于原接口的updateParameter没有返回对象，因此无法直接更新到Redis，只能在这里删除；然后再次get的时候缓存到Redis
	 * (non-Javadoc)
	 * @see net.engining.pg.parameter.JsonGuavaCachedParameterFacility#updateParameter(java.lang.String, java.lang.Object)
	 */
	@Override
	@CacheEvict(cacheNames="parameter", key="#parameter.getClass().getName() + \"_\" + #key")
	public <T> void updateParameter(String key, T parameter) {
		logger.trace("本操作将先到数据库更新该参数，如果不是内部方法调用将触发向Redis服务器删除参数缓存; param key:{}, param class:{}",
				key, parameter.getClass().getName());
	...
	public <T> void updateUniqueParameter(T parameter) {
		logger.trace("本操作将先到数据库更新该参数，如果不是内部方法调用将触发向Redis服务器删除参数缓存; param class:{}",
	 ...
	 * 由于原接口的updateParameter没有返回对象，因此无法直接更新到Redis，只能在这里删除；然后再次get的时候缓存到Redis
	 * (non-Javadoc)
	 * @see net.engining.pg.parameter.XmlGuavaCachedParameterFacility#updateParameter(java.lang.String, java.lang.Object, java.util.Date)
	 */
	@Override
	...
	public <T> void updateParameter(String key, T parameter, Date effectiveDate) {
		logger.trace("本操作将先到数据库更新该参数，如果不是内部方法调用将触发向Redis服务器删除参数缓存; param key:{}, param class:{}, effective date:{}",
				key, parameter.getClass().getName(), DateFormatUtils.format(effectiveDate, "yyyy/MM/dd HH:mm:ss"));
	...
	public <T> boolean removeParameter(Class<T> paramClass, String key) {
		logger.trace("本操作将先到数据库删除该参数，如果不是内部方法调用将触发向Redis服务器删除参数缓存; param key:{}, param class:{}",
				key, paramClass.getName());
	 ...
	 * (non-Javadoc)
	 * @see net.engining.pg.parameter.JsonGuavaCachedParameterFacility#removeParameter(java.lang.Class, java.lang.String, java.util.Date)
	 */
	@Override
	...
	public <T> boolean removeParameter(Class<T> paramClass, String key, Date effectiveDate) {
		if (ValidateUtilExt.isNotNullOrEmpty(effectiveDate)){
			logger.trace("本操作将先到数据库删除该参数，如果不是内部方法调用将触发向Redis服务器删除参数缓存; param key:{}, param class:{}, effective date:{}",
					key, paramClass.getName(), DateFormatUtils.format(effectiveDate, "yyyy/MM/dd HH:mm:ss"));
		}
