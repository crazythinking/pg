...
public class LocalCachedParameterFacility extends ParameterFacility implements ApplicationListener<ParameterChangedEvent>, InitializingBean
{
	...
	//最小日期时间+1秒，防止mysql数据库出错，timestamp型数据的取值范围('1970-01-01 00:00:00', '2037-12-31 23:59:59']
	...
	public Object convertParameterObject(String parameterStream){
	...
	public <T> void updateParameter(String key, T parameter) {
		...
		obj.setParamObject(xstream.toXML(parameter));
		obj.setMtnTimestamp(new Date());
		Authentication auth = SecurityContextHolder.getContext().getAuthentication();
		if (auth != null)
			obj.setMtnUser(auth.getName());
		...
		// 记录操作日志
		auditPrmModify(key, pok.getEffectiveDate(), pok.getParamClass(), ParamOperationDef.UPDATE, parameter, (T) xstream.fromXML(oldParamXml));
	...
	public <T> boolean removeParameter(Class<T> paramClass, String key, Date effectiveDate) {
		...
		if (paramObjects == null || paramObjects.size() == 0)
		{
			...
			for (ParameterObject paramObject : paramObjects) {
				...
				// 记录操作日志
				auditPrmModify(key, paramObject.getEffectiveDate(), paramClass.getCanonicalName(), ParamOperationDef.DELETE, null, (T) xstream.fromXML(paramObject.getParamObject()));
			}
		}
		return true;
	}
